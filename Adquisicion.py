...
#Ubicacion = 'D:/FIEC/F_BLOG/GITHUB/OpenBCI/Imagenes/RCH.jpg'
def mainsiz():
    name = "S"+Subject+"R"+str(Run)+Tipo+Siz
    im = cv2.imread('Imagenes/SupIz.jpg')  # Cambiar el nombre correspondiente
...
def mainiiz():
    ...
    im = cv2.imread('Imagenes/InfIz.jpg')
    cv2.namedWindow('Image', cv2.WINDOW_NORMAL)
    cv2.setWindowProperty('Image', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    cv2.imshow("Image", im)
    cv2.waitKey(500)
...
def mainsde():
    ...
    im = cv2.imread('Imagenes/SupDe.jpg')
    cv2.namedWindow('Image', cv2.WINDOW_NORMAL)
    cv2.setWindowProperty('Image', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    cv2.imshow("Image", im)
    cv2.waitKey(500)
...
def mainide():
    ...
    im = cv2.imread('Imagenes/InfDe.jpg')
    cv2.namedWindow('Image', cv2.WINDOW_NORMAL)
    cv2.setWindowProperty('Image', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
    cv2.imshow("Image", im)
    cv2.waitKey(500)
...
def descanso():
    img = cv2.imread('Imagenes/Descanso.jpg')
    cv2.namedWindow('Image', cv2.WINDOW_NORMAL)
    cv2.setWindowProperty('Image', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
...
while __name__ == '__main__':
    descanso()
    while valores != 20:
        valores = siz + iiz + sde + ide
        print(valores)
        opcion = str(random.randint(1, 4))
        print("aleatorio :"+ opcion)
